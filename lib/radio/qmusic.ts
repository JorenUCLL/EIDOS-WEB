import { QmusicApiChannel, QmusicApiResponse, RadioChannel } from "@/types";
...
class Qmusic extends Provider {
  async fetchChannels(): Promise<Record<string, RadioChannel>> {
    ...
    const data: QmusicApiResponse = await res.json();
    return Object.values(data.data).reduce(
      (acc: Record<string, RadioChannel>, channel: QmusicApiChannel) => {
